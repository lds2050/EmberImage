...
- 图片编辑工作流：导入 1–16 张本地图片（选择器/拖拽/粘贴），用自然语言完成整体重绘、风格转换、元素增删与多参考图合成；
- Mask 局部编辑：全屏画布涂抹希望修改的区域（画笔/橡皮、撤销/重做、缩放与拖动），导出为 API 要求的同尺寸 RGBA Mask；
- 编辑结果与文字生图共用画廊：类型与输入数量徽标、继续编辑、按原参数再次编辑、查看器原图/结果对比与编辑区域叠加；
- 非 PNG 主图在局部编辑提交前自动无损转为 PNG，用户原图不被修改；
- 连接能力状态：按实际请求结果显示生成/编辑支持情况，404/405 标记为不支持且仅作提示；
- 编辑历史保存输入图与 Mask 的独立副本，原文件移动或删除后仍可再次编辑；
- 请求日志记录编辑参数与素材元数据，不含 API Key、图片内容与完整本地路径。

V0.4.0 已实现：

- 独立“样例”菜单，按春日、雨夜、水乡、冬日分类展示 6 个内置成图案例；
- 每个样例包含压缩预览、完整提示词和生成配置，支持搜索、查看详情、复制提示词和一键复用；
- 内置样例使用轻量 WebP 预览，不把 2K/4K 原图打入安装包；
- 多份独立连接配置，可分别保存名称、Base URL、模型、密钥策略、总超时和流式开关；
- GPT Image 2 提示词、尺寸、质量、数量、背景、格式、压缩率和审核级别；
- 图像尺寸支持自动、按比例和自定义宽高；1K、2K、4K 可与 8 种常用比例独立组合，并实时显示最终像素；
