...
一个本地优先的 GPT Image 2 桌面客户端。用户输入自己的 OpenAI 或兼容 API 地址与密钥，即可通过图形界面完成文字生图。
...
- 生成中状态、取消、错误分类与请求 ID；
- 单张/批量下载、通过 Electron `ClipboardItem` 复制原图、在文件夹中显示；
- 生成结果和画廊图片双击放大预览；
...
参考图、蒙版编辑和 Responses API 多轮改图暂未开放。
...
- 设备密钥以 `0600` 权限保存在当前用户应用目录，不使用系统钥匙串。它避免 API Key 明文落盘，但不能防御已取得本机账户权限的攻击者；
- V0.2 已有的主密码记录首次升级需要解锁一次，随后自动迁移；
...
为保留 V0.1 用户数据，EmberImage 继续使用原 `gpt-image-studio` Electron `userData` 目录。单条历史的“删除”会把对应原图移到系统废纸篓；“清空记录”只清空历史索引并保留原图。
